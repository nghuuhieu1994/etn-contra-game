...
BOX DynamicObject::GetBroadPhaseBox(D3DXVECTOR2 _Velocity)
{
	BOX box(
		...
		_Velocity.x * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE)),
		_Velocity.y * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE)));
...
BOX DynamicObject::GetBroadPhaseBox()
{
	if(m_Physic->getVelocity().y != 0)
	{
		return ConvertToBroadPhase(
			...
			ConvertToBox(getBound()).width,
			ConvertToBox(getBound()).height,
			m_Physic->getVelocity().x * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE)),
			m_Physic->getVelocity().y * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE))
			)
			);
	}
	else // Lúc này gia tốc bằng 0, tăng tốc độ theo chiều âm để đảm bảo vật luôn va chạm top
	{
		return ConvertToBroadPhase(
			...
			ConvertToBox(getBound()).height,
			m_Physic->getVelocity().x * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE)),
			-300.0f * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE))
			)
			);
	}
}
...
BOX DynamicObject::GetBoundForDynamicObject(D3DXVECTOR2 velocity)
{
	return BOX(
		...
		ConvertToBox(getBound()).width,
		ConvertToBox(getBound()).height,
		velocity.x * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE)),
		velocity.y * (int) ((CGameTimeDx9::getInstance()->getElapsedGameTime().getMilliseconds() / 1000) / ((float) 1 / FRAME_RATE))
		);
}
